...
def parse_log() -> LogAnalysisResponse:
    analyze_log_1 = {
        "format": "jenkins",
        "errors": [
...
@analyze_router.post("/", response_model=LogAnalysisResponse)
async def analyze_log(upload_file: UploadFile, app_settings: AppSettings):
    if upload_file.size is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail="Log file empty"
        )

    if upload_file.size > app_settings.max_file_size_bytes:
        raise HTTPException(
            status_code=HTTP_413_CONTENT_TOO_LARGE, detail="Log file too large"
        )

    parsed = await asyncio.to_thread(parse_log)
    # after the parser is ready uncomment this lines
    # raw = await upload_file.read()

    # parsed = await asyncio.to_thread(
    #     parse_log, raw.decode("utf-8", errors="replace")
    # )
